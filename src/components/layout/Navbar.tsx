...
export function Navbar() {
  const { user, isAuthenticated, signOut, primaryRole } = useAuthStore();
          ...
          <div className="hidden md:flex items-center gap-3">
            {isAuthenticated ? (
              ...
              <>
                ...
                <Button asChild className="btn-gradient"><Link to="/auth?mode=signup">Get Started</Link></Button>
              </>
            )}
          </div>
              ...
              <div className="flex flex-col gap-2 pt-4 border-t border-border/50">
                {isAuthenticated ? (
                  <>
                    ...
                    <Button variant="destructive" onClick={() => { handleLogout(); setMobileMenuOpen(false); }}>Log out</Button>
                  </>
                ) : (
                  <>
                    <Button variant="outline" asChild><Link to="/auth" onClick={() => setMobileMenuOpen(false)}>Sign In</Link></Button>
                    <Button asChild className="btn-gradient"><Link to="/auth?mode=signup" onClick={() => setMobileMenuOpen(false)}>Get Started</Link></Button>
                  </>
                )}
              </div>
            </div>
